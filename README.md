...
filters. Server-generated row-number columns are intentionally omitted from the editable model
contract and are regenerated by the tabular engine.
...
Direct Lake does not support binary semantic columns. Model validation and the pre-deployment gate
reject them before any Fabric mutation and identify every affected table and column. Exclude those
columns from the semantic model or expose a supported source representation; the server never
silently removes a requested business column.
