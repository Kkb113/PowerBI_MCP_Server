...
export const sourceColumnSchema = z.strictObject({
  ...
  sourceProviderType: z.string().trim().min(1).max(512).optional(),
  nameInferred: z.boolean().optional(),
  dataTypeInferred: z.boolean().optional(),
  columnOriginTable: boundedNameSchema.optional(),
  columnOriginColumn: boundedNameSchema.optional(),
  description: optionalDescriptionSchema,
...
export const entityPartitionSchema = z.strictObject({
  ...
  expressionSource: boundedNameSchema,
...
const tmslCalculatedTableColumnSchema = z.strictObject({
  ...tmslColumnBase,
  type: z.literal("calculatedTableColumn"),
  sourceColumn: boundedNameSchema,
  sourceProviderType: z.string().trim().min(1).max(512).optional(),
  isNameInferred: z.boolean().optional(),
  isDataTypeInferred: z.boolean().optional(),
  isKey: z.boolean().default(false),
  columnOriginTable: boundedNameSchema.optional(),
  columnOriginColumn: boundedNameSchema.optional(),
});

// Row-number columns are internal objects generated by the tabular engine. They are accepted on
// read and omitted from ModelSpec because clients neither create nor mutate them directly.
const tmslRowNumberColumnSchema = z.looseObject({
  type: z.literal("rowNumber"),
  name: boundedNameSchema,
  dataType: dataTypeSchema,
});

const tmslColumnSchema = z.preprocess(
  (value) => {
  ...
  z.discriminatedUnion("type", [
    tmslSourceColumnSchema,
    tmslCalculatedColumnSchema,
    tmslCalculatedTableColumnSchema,
    tmslRowNumberColumnSchema,
  ]),
);
...
const tmslTableSchema = z.strictObject({
  ...
  description: optionalDescriptionSchema,
  isHidden: z.boolean().default(false),
  columns: z.array(tmslColumnSchema).default([]),
